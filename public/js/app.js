...
function setLoginMode(mode) {
  ...
  if (mode === 'admin') {
    ...
    if (DOM.loginBrandEyebrow) DOM.loginBrandEyebrow.textContent = 'Gestão TI & Telemetria';
    if (DOM.loginSystemTag) DOM.loginSystemTag.textContent = 'Hefesto v3.0';
    if (DOM.loginMainDesc) DOM.loginMainDesc.textContent = 'Painel Administrativo • Gestão Global de Parques';
    document.title = 'Hefesto — Gestão e Administração TI';
  } else {
    if (DOM.loginMainTitle) DOM.loginMainTitle.textContent = 'Prevent Senior';
    if (DOM.loginBrandEyebrow) DOM.loginBrandEyebrow.textContent = 'Prevent Senior • Gestão Hospitalar';
    if (DOM.loginSystemTag) DOM.loginSystemTag.textContent = 'Hefesto v3.0';
...
function initLoginBackground() {
  // Limpeza visual: estética hospitalar sóbria sem animação de partículas
}

function startLoginCanvas() {
  // Desativado para garantir sobriedade visual e liberar processamento da CPU
}
