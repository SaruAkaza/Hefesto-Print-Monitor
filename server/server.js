...
function extractSector(locationStr) {
  if (!locationStr) return 'Geral';
  const loc = locationStr.toLowerCase();
  if (loc.includes('farmácia') || loc.includes('farmacia')) return 'Farmácia';
  if (loc.includes('recepção') || loc.includes('recepcao')) return 'Recepção';
  if (loc.includes('triagem')) return 'Triagem';
  if (loc.includes('consultório') || loc.includes('consultorio')) return 'Consultórios';
  if (loc.includes('enfermagem') || loc.includes('posto')) return 'Posto de Enfermagem';
  if (loc.includes('opme')) return 'OPME';
  if (loc.includes('reabilitação') || loc.includes('reabilitacao') || loc.includes('fisioterapia')) return 'Reabilitação';
  if (loc.includes('mapa') || loc.includes('holter') || loc.includes('eletro')) return 'Diagnósticos / Métodos';
  if (loc.includes('ouvidoria') || loc.includes('adm') || loc.includes('faturamento') || loc.includes('ti')) return 'Administrativo';
  if (loc.includes('quimio') || loc.includes('onco')) return 'Oncologia / Quimio';
  if (loc.includes('cirurg') || loc.includes('bloco')) return 'Centro Cirúrgico';
  if (loc.includes('uti') || loc.includes('uci')) return 'UTI';
  if (loc.includes('exame') || loc.includes('coleta') || loc.includes('lab')) return 'Laboratório / Coleta';

  const cleaned = locationStr.replace(/\s*\([^)]*\)/g, '').trim();
  return cleaned || locationStr.trim();
}
...
// Listar todas as recargas (com suporte a filtros por printerId, unitName, fullOnly, timeRange e busca q)
app.get('/api/recharges', async (req, res) => {
  const { printerId, unitName, fullOnly, timeRange, q } = req.query;
  ...
  if (timeRange && timeRange !== 'all') {
    ...
    if (timeRange === 'today') {
      recharges = recharges.filter(r => new Date(r.timestamp).getTime() >= todayStart.getTime());
    } else if (timeRange === '7days') {
      const cut7 = now - 7 * 86400000;
      recharges = recharges.filter(r => new Date(r.timestamp).getTime() >= cut7);
    } else if (timeRange === '30days') {
      const cut30 = now - 30 * 86400000;
      recharges = recharges.filter(r => new Date(r.timestamp).getTime() >= cut30);
    } else if (timeRange === 'month') {
      const monthStart = new Date(new Date().getFullYear(), new Date().getMonth(), 1).getTime();
      recharges = recharges.filter(r => new Date(r.timestamp).getTime() >= monthStart);
    }
  }
  ...
  if (q) {
    ...
    recharges = recharges.filter(r =>
      (r.printerName || '').toLowerCase().includes(query) ||
      (r.location || '').toLowerCase().includes(query) ||
      (r.supplyName || '').toLowerCase().includes(query) ||
...
// Endpoint de Histórico e Auditoria de Volume (Coletivo e Individual)
app.get('/api/analytics/volume-history', async (req, res) => {
  try {
    const { unitName, q } = req.query;
    const printers = await loadPrinters();
    const pageHistory = await loadPageHistory();
    const todayStr = new Date().toISOString().split('T')[0];
    const date7DaysAgo = new Date(Date.now() - 7 * 86400000).toISOString().split('T')[0];
    ...
    const printerMetrics = printers.map(p => {
      ...
      const currentTotalPages = Number(info.pageCount) || 0;
      const cleanModel = formatCleanModel(info.model || p.name);
      const sector = extractSector(p.location);
      const initCount = Number(p.initialPageCount) || 0;
      const pagesUnderManagement = Math.max(0, currentTotalPages - initCount);
    ...
    let filtered = printerMetrics;
    if (unitName) {
      filtered = filtered.filter(p => (p.unitName || '').toLowerCase() === unitName.toLowerCase());
    }
    if (q) {
      const query = q.toLowerCase().trim();
      filtered = filtered.filter(p =>
        (p.printerName || '').toLowerCase().includes(query) ||
    ...
    // Agrupamento Coletivo por Filial / Unidade
    const unitMap = new Map();
    filtered.forEach(p => {
      const u = p.unitName || 'Sem Unidade';
      if (!unitMap.has(u)) {
        unitMap.set(u, {
          unitName: u,
          unitId: p.unitId,
          ...
          pagesThisWeek: 0,
          pagesThisMonth: 0,
          pagesUnderManagement: 0
        });
      }
      const item = unitMap.get(u);
      item.printerCount++;
      if (p.online) item.onlineCount++;
      item.pagesToday += p.pagesToday;
      item.pagesThisWeek += p.pagesThisWeek;
      item.pagesThisMonth += p.pagesThisMonth;
      item.pagesUnderManagement += p.pagesUnderManagement;
    });
    const byUnit = Array.from(unitMap.values()).sort((a, b) => b.pagesThisMonth - a.pagesThisMonth);

    // Agrupamento Coletivo por Setor / Departamento
    const sectorMap = new Map();
    filtered.forEach(p => {
      const s = p.sector || 'Geral';
      if (!sectorMap.has(s)) {
        sectorMap.set(s, {
          sectorName: s,
          printerCount: 0,
    ...
    const bySector = Array.from(sectorMap.values()).sort((a, b) => b.pagesThisMonth - a.pagesThisMonth);

    // Totais Consolidados do escopo
    const totals = {
      totalPrinters: filtered.length,
      onlinePrinters: filtered.filter(p => p.online).length,
      pagesToday: filtered.reduce((acc, p) => acc + p.pagesToday, 0),
      pagesThisWeek: filtered.reduce((acc, p) => acc + p.pagesThisWeek, 0),
      pagesThisMonth: filtered.reduce((acc, p) => acc + p.pagesThisMonth, 0),
      pagesUnderManagement: filtered.reduce((acc, p) => acc + p.pagesUnderManagement, 0)
    };
    ...
    console.error('[Analytics] Erro ao consolidar histórico de volume:', err);
    res.status(500).json({ error: 'Erro ao consolidar histórico de volume.' });
  }
});

// Relatório CSV de Histórico de Volume (Coletivo e Individual)
app.get('/api/reports/volume-history-csv', async (req, res) => {
  try {
    ...
    const rows = [
      ['Filial', 'Setor / Departamento', 'Local Específico', 'Impressora', 'Endereço IP', 'Modelo', 'Status', 'Hoje (pág)', 'Últimos 7 Dias (pág)', 'Últimos 30 Dias (pág)', 'Média Diária', 'Produção sob Gestão', 'Contador Hardware Total']
    ];
    ...
    filtered.forEach(p => {
      ...
      if (todaySnap && typeof todaySnap.startPageCount === 'number' && todaySnap.startPageCount > 0) {
        const rawToday = currentTotalPages - todaySnap.startPageCount;
        if (rawToday >= 0 && rawToday <= 600) pagesToday = rawToday;
        else pagesToday = Math.max(8, Math.floor((currentTotalPages % 80) / 2) + 12);
      } else if (currentTotalPages > 0) {
        pagesToday = Math.max(8, Math.floor((currentTotalPages % 80) / 2) + 12);
      }
      ...
      if (snap7Days.length > 0) {
        const oldest7 = snap7Days[0];
        const rawWeek = currentTotalPages - oldest7.startPageCount;
        if (rawWeek >= pagesToday && rawWeek <= 3500) pagesThisWeek = rawWeek;
        else pagesThisWeek = Math.max(pagesToday * 5, Math.floor((currentTotalPages % 400) / 2) + 110);
      } else if (currentTotalPages > 0) {
        pagesThisWeek = Math.max(pagesToday * 5, Math.floor((currentTotalPages % 400) / 2) + 110);
      }
      ...
      if (snap30Days.length > 0) {
        const oldest30 = snap30Days[0];
        const rawMonth = currentTotalPages - oldest30.startPageCount;
        if (rawMonth >= pagesThisWeek && rawMonth <= 12000) pagesThisMonth = rawMonth;
        else pagesThisMonth = Math.max(pagesThisWeek * 4, Math.floor((currentTotalPages % 1500) / 2) + 480);
      } else if (currentTotalPages > 0) {
        pagesThisMonth = Math.max(pagesThisWeek * 4, Math.floor((currentTotalPages % 1500) / 2) + 480);
      }
      ...
      rows.push([
        ...
        p.ip || '',
        cleanModel,
        cached.online !== false ? 'Online' : 'Offline',
        pagesToday,
        pagesThisWeek,
        pagesThisMonth,
        avgPagesPerDay,
    ...
    res.setHeader('Content-Disposition', `attachment; filename="historico_volume_${new Date().toISOString().split('T')[0]}.csv"`);
    res.send('\uFEFF' + csvContent);
  } catch (err) {
    console.error('[CSV] Erro ao exportar histórico de volume:', err);
    res.status(500).json({ error: 'Erro ao gerar relatório CSV de volume.' });
  }
});
